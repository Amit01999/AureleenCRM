...
import { CrmLaunchLink } from "./crm-launch-link";
  ...
  return (
    <AppShell>
      <section className="product-hero page-width">
        <div className="product-hero-copy">
          ...
          <div className="hero-actions">
            ...
            <Button asChild variant="outlineLight" size="lg">
              <a href="mailto:hello@aureleen.com?subject=Product%20demonstration">
                Request a demonstration <ArrowUpRight />
              </a>
            </Button>
          </div>
        </div>
      ...
      <section className="problem-solution page-width">
        <article>
          <span>01 / Industry problem</span>
          <h3>Fragmented work creates fragmented visibility.</h3>
          <p>{product.problem}</p>
        </article>
        <article>
          <span>02 / System response</span>
          <h3>One structured operating context.</h3>
      ...
      <section className="system-map page-width">
        <SectionIntro
          ...
          title="A system map, not a feature checklist."
          body="The architecture groups related operational capabilities into one connected product environment."
        />
        ...
        <div className="workflow-track">
          {product.workflow.map((step, index) => (
            <div key={step} className="workflow-step">
              <span>{String(index + 1).padStart(2, "0")}</span>
              <strong>{step}</strong>
              {index < product.workflow.length - 1 && <ArrowRight />}
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="interface-showcase" id="interface">
        <div className="page-width">
          <SectionIntro
            ...
            title="The system becomes the working environment."
            body="Explore representative interface structures designed for focused operational work. Final product imagery can replace these views without changing the presentation system."
          />
          <Tabs defaultValue={product.screens[0].name} className="screen-tabs">
            <TabsList>
              {product.screens.map((screen) => (
                <TabsTrigger key={screen.name} value={screen.name}>
                  {screen.name}
                </TabsTrigger>
              ))}
            </TabsList>
            {product.screens.map((screen) => (
              <TabsContent key={screen.name} value={screen.name}>
      ...
      <section className="roles-section page-width">
        <SectionIntro
          ...
          title="One system. Different operational views."
          body="Each role works from the same underlying records while seeing the responsibilities, information and actions relevant to its work."
        />
        <div className="role-chain">
          {product.roles.map((role, index) => (
            <article key={role.name}>
              <span>{String(index + 1).padStart(2, "0")}</span>
              <h3>{role.name}</h3>
              <p>{role.focus}</p>
              {index < product.roles.length - 1 && <ArrowRight />}
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="benefits-section page-width">
        <SectionIntro
          ...
          title="Designed for continuity at every level."
        />
        ...
        <div className="architecture-stack">
          {[
            ...
            <div key={layer}>
              <span>0{index + 1}</span>
              <strong>{layer}</strong>
              <small>{index === 4 ? "System foundation" : "Connected layer"}</small>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="related-products page-width">
        ...
        <div>
          {otherProducts.map((item) => (
            <Link key={item.id} to="/products/$slug" params={{ slug: item.slug }}>
